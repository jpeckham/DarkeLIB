...
void spell_func(object caster, object at, int power, string args, int flag) {
  ...
  object ob;
  seteuid(getuid());
  if((int)at->query_hp()-100 > (int)at->query_max_hp()) {
    message("info", "You can't cast enlarge because hp is too high.", at);
  ...
  roll = 0;
  for(i=1;i<5;i++) roll += random(20) + random(20) + power;
  roll *= power;
    at->set_property("allow_over_max",1);
  at->add_hp(roll);
    at->set_property("allow_over_max",0);
