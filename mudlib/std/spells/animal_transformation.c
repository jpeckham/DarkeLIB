...
void create() {
    ::create();
    set_property("name","animal transformation");
    ...
    set_property("prereq", "alter visage");
    return;
}
...
void spell_func(object caster, object at, int power, string args, int flag) {
  ...
  if(caster->query_lyc_status()) {
    message("info", "You cannot cast this spell while transformed by lycanthropy.", caster);
  ...
  i = sizeof(inv);
  while(i--) if(inv[i]->query_short() && !inv[i]->drop()) inv[i]->move(environment(caster));
  ...
  ob->start_shadow(caster, props["duration"], "%^CYAN%^Animal transformation wears off.");
