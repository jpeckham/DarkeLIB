...
void create() {
    ::create();
    set_property("name","mirror image");
    ...
    set_property("caster message","You cast mirror image at $T.");
    set_property("target message","$C casts mirror image at you.");
    set_property("observer message","$C casts mirror image at $T.");
...
void spell_func(object caster, object at, int power, string args, int flag) {
  ...
  message("info", "%^GREEN%^%^BOLD%^"+capitalize(consolidate(power +2, "mirror image of "+(string)at->query_cap_name()))+
                  " appear.", environment(at));
  if(flag)
    message("info", "%^RED%^%^BOLD%^The mirror images attack "+
        (string)at->query_cap_name()+"!", environment(at));
  remove();
  return;
}
