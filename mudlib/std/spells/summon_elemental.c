...
void create() {
    ::create();
    set_property("name","summon elemental");
    ...
    set_property("observer message","$C begins to magically mold a clay figurine.");
    set_property("spell type",({ }));
    set_property("target type", "any");
    set_property("must be present", 1);
    set_property("prereq", "summon lesser elemental");
    return;
}
...
void spell_func(object caster, object at, int power, string args, int flag) {
  if(!at->id("clay figurine")) {
    message("info", "You must cast this spell at a clay figurine.",
	    caster);
    remove();
    return;
  }
...
void finish_work(object caster, object at, int power) {
  ...
  message("info", "The clay figurine transforms into a "+props["element"]+
	  " elemental.", caster);
  message("info", (string)caster->query_cap_name() + " utters some "+
	  "magical incantations, and a clay figurine "+
