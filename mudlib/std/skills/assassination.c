...
"strike.  This skill is more potent that 'back stab' in that "
"it does more damage, penetrates armour better, and is more "
"difficult to defeat.  It cannot be used while in combat.  Watch out "
...
void skill_func(object from, object at, string arg) {
  ...
  roll = skill_contest(props["skill level"],
		       (int)at->query_skill("reverse stroke")+bonus, 1);
  if(roll == 2) {
    wc = 0;
  ...
  roll = skill_contest(props["skill level"],
		       (int)at->query_skill("perception")+bonus, 1);
  if(roll == 2) {
...
void do_stab(object from, object at, mapping wc, int skill) {
  ...
  while(i--) {
    ...
    if(dmg < 1) dmg = 1;
    dmg = (int)at->do_damage("torso", dmg*2);
    if(dmg >= 0 && wc_keys[i] == "impaling") {
      mesgs = (string *)DAMAGE_D->attack_message(sprintf(
                "%d %s %s:%s", dmg/2, ((rev_flag)?"reverse-strike":"assassinate"),
    ...
    switch(wc[wc_keys[i]] + skill - random(140)) {
    case -50..0:
      crits += ({ wc_keys[i] + " A" });
      break;
    case 1..20:
