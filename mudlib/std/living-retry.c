...
//ADDED NEW DEV SYS TO HERE (parnell feb99)
       if(!query("no add dev") && dev_rate_int = get_dev_rate() > 0)
		{
if(!xp_to_next_dev || xp_to_next_dev <= 1) xp_to_next_dev = player_data["general"]["max exp"] + dev_rate_int;
...
if(next_lev_exp <= 0) next_lev_exp -= (next_lev_exp - next_lev_exp);
//yes this all looks stupid but you can thank Diewarzau's fucking advance_D for this shit, his code sucks
// My left nut and then some... so bear with me.
	  if(GDlev < 10) {
