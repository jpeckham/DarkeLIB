...
private void delete_duplicates();
...
void create() {
  ...
  if(!this_player() && file_exists(DIR_SECURE_DAEMONS_SAVE+
				   "/unreg_at_reboot.v")) {
      unreg_at_reboot = restore_variable(read_file(DIR_SECURE_DAEMONS_SAVE+
					     "/unreg_at_reboot.v"));
      rm(DIR_SECURE_DAEMONS_SAVE+"/unreg_at_reboot.v");
  }
  for(i=0; i<10; i++) {
...
int register_objects(object *inv, object ob) {
  ...
  if(!i) {
    if(obj_register[oidx] && obj_register[oidx][l_file]) {
...
private void manage_lockers() {
  string *dir, file, *p_str, ofile, rfile, lfile;
  mapping tmp, blist, lkr;
  class uil tuil;
  string obj_reg = "";
  int i, idx;
  string key;
  ...
  /*
   * First, unregister all duplicate objects in the 
   * obj register.
   */
  tuil = find_uid_list("obj");
  if(!tuil) {
    seteuid(UID_LOG);
    log_file("update_lockers", "Obj register NOT FOUND...no update!\n");
    return;
  }
  for(i=0;i<10;i++) {
    tmp = copy(obj_register[i]);
    if(!tmp || !mapp(tmp)) continue;
    foreach(lfile, lkr in tmp) {
      p_str = ({});
