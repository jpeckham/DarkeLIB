...
void setup() {
    ...
    if(member_array(query_position(), MORTAL_POSITIONS) == -1) {
        log_file("driver/seclog", "enable_wizard^"+this_object()->query_name()+"^"+getuid(previous_object()) + "^" + ctime(time()) + "^" +this_object()->query_ip()+"^SETUP\n");
	enable_wizard();
    }
    init_living();
    basic_commands();
...
void set_position(string pos) {
    int ovrd = 0;
    log_file("driver/seclog", "set_position^"+this_object()->query_name()+"^"+getuid(previous_object()) + "^" + ctime(time()) + "^" +this_object()->query_ip());
    if(geteuid(previous_object()) != UID_ADVANCE && geteuid(previous_object()) != UID_ROOT) {
      log_file("driver/seclog", "^FAIL"+"\n");
      return;
    }
    if(member_array(position, MORTAL_POSITIONS) != -1)
	if(member_array(pos, MORTAL_POSITIONS) == -1) {
	    ovrd = 1;
	    log_file("driver/seclog", "^SUCCESS"+"\n");
            log_file("driver/seclog", "enable_wizard^"+this_object()->query_name()+"^"+getuid(previous_object()) + "^" + ctime(time()) + "^" +this_object()->query_ip()+"^SET_POSITION\n");
	    enable_wizard();
	}    
    ...
    init_path();
    position = pos;
    if(!ovrd) log_file("driver/seclog", "^SUCCESS"+"\n");
    return;
} 
