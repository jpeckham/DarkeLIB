// /d/nfd/newbie/rooms/fr2.3

inherit "std/room";

int check_mobs();

// we will call the monster file /d/nfd/newbie/mon/teddy_bear,
// and the room /d/nfd/newbie/rooms/fr2.3
...
void reset() {
	...
	if(n <= 0) return;
	while(n > 0) {
		new("/d/nfd/newbie/mon/teddy_bear")->move(this_object());
		n--;
	}
}
...
void create() {
	...
	set_exits((["northwest":"/d/nfd/newbie/rooms/fr1.2",
		"northeast":"/d/nfd/newbie/rooms/fr3.2",
		"south":"/d/nfd/newbie/rooms/fr2.4"]));	
...
int my_mob_filter(object mob) {
  if(base_name(mob) == "/d/nfd/newbie/mon/teddy_bear") 
    return 1;                                     
  return 0;                                       
}
