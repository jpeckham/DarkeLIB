...
void reset() {
	::reset();
	if (!present("toastmaster")) new("/d/nfd/mon/toaster_king")->move(
			this_object());
	if (!present("joey")) { new("/d/nfd/mon/living_toaster")->move(
			this_object());
...
set_exits( ({"/d/nfd/rooms/sewer3.7"}), ({"east"}) );
}
