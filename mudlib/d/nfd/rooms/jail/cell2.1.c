...
void reset() {
	...
	set_open("cell 1",0);
	set_locked("cell 1",1);
	"/d/nfd/rooms/jail/prison2"->set_open("cell 1",0);	
	"/d/nfd/rooms/jail/prison2"->set_locked("cell 1",1);
...
void create() {
	...
	set_items((["toilet":"It is out in the open for guards to see.",
		({"cell door","door","cell"}):"It is made out of bars that guards can "+
			"see right through."]));
	set_exits((["south":"/d/nfd/rooms/jail/prison2"]));
	set_door("cell 1","/d/nfd/rooms/jail/prison2",
		"south","jail keys");
