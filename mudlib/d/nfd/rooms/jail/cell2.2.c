...
void reset() {
	object guard;
	if(!present("guard")) 
		new("/d/nfd/mon/whip_guard")->move(this_object());
	if(!present("prisoner"))
		new("/d/nfd/mon/p_prisoner")->move(this_object());
	present("guard")->set_money("gold",300);
	set_open("cell 2",0);
	set_locked("cell 2",1);
	"/d/nfd/rooms/jail/prison2"->set_open("cell 2",0);	
	"/d/nfd/rooms/jail/prison2"->set_locked("cell 2",1);
}
...
void create() {
	...
	set_exits((["north":"/d/nfd/rooms/jail/prison2"]));
	set_door("cell 2","/d/nfd/rooms/jail/prison2",
		"north","jail keys");
