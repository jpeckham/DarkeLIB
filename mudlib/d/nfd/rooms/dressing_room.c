...
void reset() {
	...
	new("/d/nfd/mon/actress.c")->move( this_object() );
}
...
void create() {
	...
	set_exits( (["east":"/d/nfd/rooms/dressing_rooms"]) );
	set_door("door","/d/nfd/rooms/dressing_rooms","east","star key");
...
int jump_pipe(string str) {
	if (str == "to pipe" || str == "pipe" || str == "to drainpipe" || str ==
		...
		this_player()->move_player( "/d/nfd/rooms/pipe" );
		return 1;
	}
	else
	{
