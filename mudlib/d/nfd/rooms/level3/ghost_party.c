// /d/nfd/rooms/level3/ghost_party
// whee
// Excelsior 6-26-96
// lot o' sixes ^^^^
...
void reset() {
	::reset();
	if(!present("galran")) 
		new("/d/nfd/mon/galran")->move(this_object());
	if(!present("poltergeist")) 
		new("/d/nfd/mon/poltergeist")->move(this_object());
	if(!present("poltergeist 2")) 
		new("/d/nfd/mon/poltergeist")->move(this_object());
	if(!present("drunk")) 
		new("/d/nfd/mon/drunk_ghost")->move(this_object());
	if(!present("skeleton"))
		new("/d/nfd/mon/skeleton")->move(this_object());
	chest_opened = 0;
	can_open = 0;
}
...
void create() {
	...
	set_items(([({"ghosts","spiritual apparitions","ghouls","undead creatures"}):
		...
		"throne":"It looks very neat. You wonder who sits on it.",
		"chest":(: call_other, this_object(), "look_at_chest":)]));
	set_exits((["east":"/d/nfd/rooms/level3/hall2"]));
