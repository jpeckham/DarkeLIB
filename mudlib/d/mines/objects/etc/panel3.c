...
void create() {
 ...
 "There is also a small plate with writting.");
    set_weight(100);
    set_value(70);
    x = " ";
    z = " ";
...
write("Each lever row has three possible horizontal positions.\n" 
   ...
   if(str == "plate"){
    ...
    "'The proper lever configuration for section 51 is lever 1 ...'\n\n"
    ...
    if(lever == "button" || lever == "red button") { 
     ...
     if(c1 == p1 && c2 == p2 && c3 == p3){
       ...
       if(!present("cart")){
         ...
         "distance.");  
         player = this_player();
         call_other(UNDER+"dm9c","bring_cart");
         return 1;
       }
       if(present("fcart")){
        tell_room(here,"You hear a small clank, and the cart rolls out of "
        "the room.");
        present("fcart")->move(UNDER+"dm9c");
        tell_room(UNDER+"dm9c","A large rusty mining cart rolls into the "
