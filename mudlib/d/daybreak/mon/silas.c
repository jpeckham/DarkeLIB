...
void create() {
    ...
    ::create();
    set_name("silas");
    set("id", ({"silas", "moneychanger", "money changer"}));
    ...
    set_max_hp(2000000);
    set_exp(2000000);
    set("short","Silas, the moneychanger");
    set("long",
      "Silas is a straight-laced looking half-elf approaching middle age.  "
      "Oddly, he wears a wide-brimmed hat inside, and it conceals his eyes.");
    set_gender("male");
    set_body_type("human");
    set("race", "half-elf");
...
//	write("As far as I'm concerned, "+what1+" isn't worth a thing!");
//	return 1;
//    }
    ...
    if(!to_int(rate)) {
	write("I'm sorry, but "+amt+" "+what1+" is worth less than a "+
	  "single "+what2+".");
	return 1;
    }
    this_player()->add_money(what1, -1 * amt);
    this_player()->add_money(what2, to_int(rate));
    write("Your exchange is worth "+to_int(rate)+" "+what2+".\n"+
      "Here you go.");
    return 1;
}
