// even more massively stolen code by nEv!N
// Original Code: Darkemud Lib
...
void save_this_room() {
  ...
  save_object("/d/nocte/kuril/virtual/inn_room");
  seteuid(getuid());
  return;
}
...
int query_time_left(string owner) {
  ...
  if(member_array(owner, query_all_owners()) < 0)
    return 0;
  room_file = sprintf("/d/nocte/kuril/virtual/inn_%d.inn",
...
void move_all_inv(object room_obj) {
  ...
  if(room_obj) {
	...
	inv[i]->move(load_object("/d/nocte/kuril/rooms/general_store_storage"));
    }
  }
  return;
}
...
int get_next_room() {
  ...
  for(j=0;j < max; j++) {
    num = sprintf("%d%2'0'd", j/ROOMS_PER_FLOOR + 1,
                  j%ROOMS_PER_FLOOR);
    if(!owners || !owners[sprintf("/d/nocte/kuril/virtual/inn_%s.inn", num)])
      return atoi(num);
  }
  return 0;
}
...
void create() {
  ::create();
  owners = ([]);
  restore_object("/d/nocte/kuril/virtual/inn_room");
  set_property("indoors", 1);
  set_property("light", 2);
  set_property("no attack", 1);
  set_property("no spell", 1);
  set("short", "Reception Desk of the Wretched Zombie Inn");
  set("long",
	"Not the finest inn you have ever seen, but the floors appear to "
	"be cleaned regularly and there are no dead bodies lying about.  "
	"Such luxuries are rare in this city.  Perhaps this inn is not "
	"so bad after all.  There is a sign on the wall which has the "
	"inn's rates.  Try telling the clerk 'help' for info on renting "
	"rooms.\n"
...
void reset() {
  ...
  if(!present("clerk", this_object())) {
    seteuid(getuid());
    ob = new("/d/nocte/kuril/inn/inn_clerk");
    ob->move(this_object());
}
