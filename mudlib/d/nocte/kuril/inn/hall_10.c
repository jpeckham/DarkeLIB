...
create() {
  ...
  for(i = rn*10; i < (rn*10+10); i++) {
    add_exit(sprintf("/d/nocte/kuril/virtual/inn_%d.inn", i),
	     sprintf("room%d", i));
    set_door(sprintf("door %d", i), 
      sprintf("/d/nocte/kuril/virtual/inn_%d.inn", i),
  ...
  add_exit("/d/nocte/kuril/inn/main_room", "down");
	add_exit("/d/nocte/kuril/inn/h11", "east");
	add_exit("/d/nocte/kuril/inn/hall_20", "up");
  return;
}
