...
#include <security.h>
#include <daemons.h>
#define INN_ROOM "/d/nocte/kuril/inn/main_room"
...
void virtual_setup(string arg) {
  ...
  owner =
(string)INN_ROOM->query_owner(sprintf("/d/nocte/kuril/virtual/inn_%d.inn",
						room_num));
  seteuid(getuid());
// INN DESCS DIRECTORY
  if(owner && file_exists(file = ("/d/nocte/kuril/inn/inn_descs/"+owner)))
    set("long", read_file(file));
  hall_exit = sprintf("/d/nocte/kuril/inn/hall_%d", (room_num / 10));
...
int change_desc(string str) {
  ...
  write("-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-\n");
  seteuid(getuid());
// MAY NEED TO CHANGE THIS DIR FOR PERMISSIONS
  file = "/d/nocte/kuril/inn/inn_descs/"+(string)this_player()->query_name();
