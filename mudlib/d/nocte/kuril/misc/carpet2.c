...
string query_short() {
	if(ROLLED) { return "A rolled up large luxuriant carpet"; }
	return "An unrolled large luxuriant carpet";
}

string query_long() {
	if(ROLLED) { 
		return "A large luxuriant carpet.  It would be a very nice carpet to "
			"lay on top of and relax.  You should probably "
			"<lay> it out first."; 
	}
	return "A large luxuriant carpet which has been unrolled for you to relax "
