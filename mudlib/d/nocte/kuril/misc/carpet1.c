...
string query_short() {
	if(ROLLED) { return "A rolled up large carpet"; }
	return "An unrolled large carpet";
}

string query_long() {
	if(ROLLED) { 
		return "A large carpet.  It would be a very nice carpet to "
			"lay on top of and relax.  You should probably "
			"<lay> it out first."; 
	}
	return "A large carpet which has been unrolled for you to relax "
