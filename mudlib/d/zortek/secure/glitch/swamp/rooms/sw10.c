...
void create() {
	...
	set("long", "You're walking on a path through a swamp.  Trees lay "+
		     ...
		     "grass stick up here and there.  A large fallen tree "+
		     "spans a large open expanse of water to the north.");
	add_exit("sw7","southwest");
	add_exit("sw11","north");
...
void reset() {
	::reset();
	if(!present("shambling mound", this_object()) ) {
		new("/wizards/glitch/swamp/mons/shambler.c")->move(this_object());
	}
}
