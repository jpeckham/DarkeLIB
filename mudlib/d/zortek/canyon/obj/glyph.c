//    A generic glyph
//    Adapted from Diewarzau's Runes
#include <move.h>
#include <rooms.h>
#include <daemons.h>
...
void init() {
  ...
  ::init();
  message("info","Line 93...glyph.c", environment(this_object()));
  if((string)this_player()->query_name() == caster) {
    add_action("erase_glyph", "erase");
    return;
  }
  message("info","Line 98...glyph.c", environment(this_object()));
  if(!functionp(go_off=query("function"))){
    return;
  }
  message("info","Line 102...glyph.c", environment(this_object()));
  ...
  message("info","Line 114...glyph.c", environment(this_object()));
  this_object()->go_off(this_player());
//  (*go_off)(this_player());
  return;
}
