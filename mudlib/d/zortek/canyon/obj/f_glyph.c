...
//    Zortek 960322
#define CRITS (["1":({"fire A"}),"2":({"fire A","fire A"}),\
  "3":({"fire B","fire A"}),"4":({"fire B","fire B"}),\
  "5":({"fire C","fire B"}),"6":({"fire C","fire C"})])
...
void create() {
  ...
  level = 1;
  non_active = ({});
  set_id(({ "fiery glyph", "soulburn glyph", "glyph" }));
  set_short("%^BOLD%^MAGENTA%^A %^RED%^BOLD%^soulburn glyph%^MAGENTA%^BOLD%^");
  set_long(@GLYPH
...
void init() {
  ...
  this_object()->do_burn(this_player());
  return;
}
