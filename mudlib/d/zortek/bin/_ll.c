/*  /cmds/system/_ls.c
 *  Made for DarkeMUD; use in other MUDs is prohibited
 *  Created by Misery 032096
 *  Copyright (c) 1996
...
varargs int do_ls(mixed path, object act_ob, int show_dots, string real_path)
{
	...
	message("info", temp, this_player());
    }
#if 0
    if (index % cols != 0)
	write( "\n" );
#endif
    write( "A total of " + tsize + " bytes in " + tfiles +
      " file" + ((tfiles != 1) ? "s." : ".") +
      "  (" + tdirs + " director" + ((tdirs != 1)?"ies)":"y)") + "\n", this_player() );
    return 1;
}

int cmd_ll( string path )
{
    string *Files,*Dirs;
    int i, num;
    string *wcfiles;
    ...
    seteuid(getuid(act_ob));
    if(path && (path == "-l" || sscanf(path,"-l %s",str2)))  {
	act_ob->more((string *)LSX->lsl(str2), "system");
	return 1;
    }
