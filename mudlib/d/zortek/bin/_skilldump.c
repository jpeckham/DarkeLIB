#include <std.h>
#define EXCEPTS ({"skill.c","long_term_skill.c"})

inherit DAEMON;

void do_dump(string path);

int cmd_skilldump(string str)
{
   string path,file;
...
void do_dump(string path)
{
   ...
   files = get_dir(path+"/");
    ...
    write_file("/wizards/zortek/skill_list.txt", "Name\tSkill\tLevel\tTime\tMana\tDev\tFast Dev\tSpell Type\tNo Target");
