...
void create() {
  ::create();
	...
	set_id(({"skull"}));
	set_short("A strange skull");
	set_long("A skull with a candle on top to lighten your shoulders");
    set_weight(100);
    set_float_value(0.1);
	fuel = 100000;
    lit = 0;
...
int light(string str) {
	...
	write ("You light the skull.\n");
    say(this_player()->query_cap_name()+" lights a torch.\n", this_player());
	environment(this_object())->set_property("light", 4);
