...
"Syntax: use inscribe malsa bonnes at <weapon>",
	this_player());
}
...
void finish_work(object from, object at) {
  ...
  message("info", "A %^BLUE%^%^BOLD%^Malsa Bonnes%^RESET%^ rune appears on the weapon.", from);
  if(!(runes=(string *)at->query_property("runes")))
    runes = ({});
  runes += ({ "malsa bonnes" });
  at->set_property("runes", runes);
  tmp = at->query_property("extra long");
  if(pointerp(tmp)) tmp += ({ "A %^BLUE%^%^BOLD%^Malsa Bonnes%^RESET%^ rune is inscribed on the weapon." });
  else if(stringp(tmp)) tmp = ({ tmp, "A %^BLUE%^%^BOLD%^Malsa Bonnes%^RESET%^ rune is inscribed on the weapon." });
  else tmp = ({ "A %^BLUE%^%^BOLD%^Malsa Bonnes%^RESET%^ rune is inscribed on the weapon." });
