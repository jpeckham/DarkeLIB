...
create() {
  ::create();
    ...
    set_short("A big, burly guard");
     set("race", "high-man");
    set_long("The guard stands in front of the steps down.");
    set_body_type("human");
    set_level(18);
    set_skill("melee", 60);
    set_skill("dodge", 70);
...
int block_exit() {
  ...
  if((string)this_player()->query_class() != "arch-mage") {
    write("The Guard says in Common: Only arch-mages may pass.");
    return 1;
  }
  inv = filter_array(all_inventory(this_player()), "locker_filter",
        this_object());
  if(sizeof(inv)) {
    write("The Guard says in Common: We do not allow storage "
      "devices in here.");
    return 1;
  }
  gld = 5 * (int)this_player()->query_level();
  if((int)this_player()->query_money("silver") < gld) {
    write("The Guard says in Common: A donation of "+gld+" silver "
      "is required for passage.");
    return 1;
  }
   join_room = 
     find_object_or_load("/d/damned/guilds/join_rooms/arch-mage_join");
   p_name = (string)this_player()->query_name();
   if(member_array("treasury restrict", (string *)join_room->
     query_member_privs(p_name)) >= 0) {
       write("The Guard says in Common: You have been barred from "
         "the treasury by the Guildmaster.");
       return 1;
   }
