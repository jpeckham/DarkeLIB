...
void reset() {
   ::reset();
   seteuid(getuid());
   if (!present("dakkon") && open) {
      new("/d/damned/akkad/obj/mon/dakkon")->move(this_object());
   }
}
...
void create() {
   ...
   add_exit("/d/damned/akkad/ak_markne", "west");
   add_exit("/d/damned/akkad/shops/shop7_store", "east");
