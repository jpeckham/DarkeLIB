...
void directed_message(object who, string str) {
  ...
  if( (str == "I desire to be blessed" || str == "Idesiretobeblessed") && who) {
    ...
    else if( test_bit(guild_security, ALLOW_TO_JOIN) ) {
      force_me("speak "+who->query_primary_lang());
  ...
  if( (str == "remove the blessing" || str == "removetheblessing") && who) {
    ...
    GUILD2_D->rm_guest(GUILD_NAME, who->query_name());
    this_player()->set_guild_security(clear_bit(guild_security, ALLOW_TO_JOIN));
    this_player()->remove_property("guild allowing");
    force_me("speak "+who->query_primary_lang());
...
void question_allow(string str, object who){
  ...
  if(str == "yes") {
    ...
    this_player()->set_guild_security(set_bit(guild_security, ALLOW_TO_JOIN));
    this_player()->set_property("guild allowing", GUILD_NAME);
