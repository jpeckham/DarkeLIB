...
#include "/d/damned/guilds/cleric_new/guild.h"    // Global Guild include file
 ...
 */
mapping query_regen(string guild, string what) {
  return regen[guild];
}
...
mapping query_advance(string guild, string what) {
...
//  seteuid(getuid());
  if(file_exists("/d/damned/guilds/cleric_new/guild2_d.o"))
    restore_object("/d/damned/guilds/cleric_new/guild2_d",1);
  return;
}
void reset() {
//  seteuid(UID_SECURE_DAEMONSAVE);
...
//  seteuid(getuid());
  save_object("/d/damned/guilds/cleric_new/guild2_d");
  return;
}
