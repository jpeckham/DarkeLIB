...
//    This is a hack to provide a donation box for clerics
//    DarkeLIB 0.1
//    -Diewarzau 7/29/95
//    Zortek
...
void create() {
  ...
  set_long("This is a donation box that holds the cash donated to the church.\n");
  set_weight(100000);
  set_value(10000);
  set_closed_long("This is a donation box that holds the cash donated to the church for services provided by its members to the general public.\n");
  set_open_long("This is an open donation box that holds the cash donated to the church for services provided by its members to the general public.\n");
}
...
int get_from(string arg) {
  ...
  if(this_object()->query_closed()) {
    write("Divine inspiration reveals that the donation box is closed.");
    return 1;
  }
...
int put_into(string str) {
  ...
  add_money(what, amt);
  write("You donate "+amt+" "+what+" to the church.");
    "/daemon/chat"->send_chat("cleric", "Cardinal Philip",
      (string)this_player()->query_cap_name()+" just donated "+amt+" "+what+" to the church.");
    if( amt > 10000 )
      "/daemon/chat"->send_chat("darke", "Cardinal Philip",
        (string)this_player()->query_cap_name()+" just donated "+amt+" "+what+" to the church.");
  return 1;
