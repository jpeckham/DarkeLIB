...
"The guard stands in front of the east exit.");
    ...
    set_skill("block", 115);
    set_property("extra hp regen", 100);
    set_skill("blade", query_skill("blade")+20);
...
int block_exit() {
  ...
  if(wizardp(this_player())) return 0;
   if((string)this_player()->query_class() != "fighter") {
     write("The Guard says in Common: Only fighters may pass");
    return 1;
  }
 ...
 join_room = find_object_or_load("/d/damned/guilds/join_rooms/fighter_join");
